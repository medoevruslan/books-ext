...
function findInputByLabels(labelArray) {
  for (const text of labelArray) {
    const elems = [...document.querySelectorAll('*')]
    const elem = elems.find(el => el.textContent.trim() === text);
    if (elem) {
      return elem.parentElement.querySelector('input, textarea, [contenteditable]');
    }
  }
  return null;
}
...
const publisher = findInputByLabels(LABELS.publisher);
if (publisher) {
  publisher.focus();
